...
fn last_two<T>(values: &[T]) -> &[T] {
    let two_from_end = values.len() - 2;
    &values[two_from_end..]
}
...
// 4. find_string_that_has_content
...
fn main() {
    ...
    println!("Double the length: {}", double_the_length(&numbers));

    let letters = vec!['a', 'b', 'c', 'd', 'e', 'f'];
    println!("Last two letters: {:?}", last_two(&letters));

    let text = "refrigerator";
    let announcement = "Hello!";
    println!("First five characters: {}", first_five(text, announcement));

    let first_string = "programming";
    let second_string = "dining";
    let target_substring = "gram";
    println!(
        "String that contains '{}': {}",
        target_substring,
        find_string_that_has_content(first_string, second_string, target_substring)
    );
}
