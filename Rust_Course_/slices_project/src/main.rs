fn main() {
    ...
    println!("this is the first two cereals: {first_two:?}");
    let mid_three = &cereals[1..4];
    println!("this is the mid three cereals: {mid_three:?}");
    let last_three = &mut cereals[2..];
    println!("this is the last three cereals: {last_three:?}");
    last_three[2] = String::from("Lucky Charms");
    println!("this is a replacement: {last_three:?}");
    println!("this is a the full cereals array now: {cereals:?}");

    let cookie_crisp = &cereals[0];
    println!("this is variable that references the Cookie Crisp: {cookie_crisp}");
    let cookie = &cookie_crisp[..6];
    println!("a slice of the text: {cookie}");

    let cocoa_puffs = &cereals[3];
    println!("this is variable that references the cocoa puffs: {cocoa_puffs}");
    let puffs = &cocoa_puffs[6..];
    println!("a slice of the text: {puffs}");
